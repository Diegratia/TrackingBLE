...
using TrackingBle.src._17MstOrganization.MappingProfiles;
using DotNetEnv;
...
builder.Services.AddDbContext<MstOrganizationDbContext>(options =>
...
var port = Environment.GetEnvironmentVariable("MST_ORGANIZATION_PORT") ??
           builder.Configuration["Ports:MstOrganizationService"] ?? "5017";
...
if (app.Environment.IsDevelopment())
{
    ...
    app.UseSwaggerUI(c =>
    {
        c.SwaggerEndpoint("/swagger/v1/swagger.json", "MstOrganization API V1");
        c.RoutePrefix = "";
    });
}
...
app.MapGet("/api/MstOrganization/health", () => "Health Check");
// app.MapGet("/", () => "Health Check");

Console.WriteLine("Environment Variables Check");
Console.WriteLine($"ASPNETCORE_ENVIRONMENT: {env}");
Console.WriteLine($"MST_ORGANIZATION_PORT: {Environment.GetEnvironmentVariable("MST_ORGANIZATION_PORT")}");
